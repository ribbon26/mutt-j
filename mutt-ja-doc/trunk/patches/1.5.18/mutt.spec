...
Patch11: mutt-1.5.18-create_params.patch
Patch12: mutt-1.5.18-delete_prefix.patch
Patch13: mutt-1.5.18-wcwidth.patch
...
%if %{hcache}
%{?_with_bdb:BuildRequires: db4-devel}
%{?_with_qdbm:BuildRequires: qdbm-devel}
%{?_with_gdbm:BuildRequires: gdbm-devel}
%endif
%if %{imap} || %{pop} || %{smtp}
%{?gnutls:BuildRequires: gnutls-devel}
%{?_with_sasl:BuildRequires: cyrus-sasl-devel}
%endif
%if %{imap}
%{?_with_gss:BuildRequires: krb5-devel}
%endif
%{?_with_idn:BuildRequires: libidn-devel}
...
%patch6 -p1 -b .imap
%patch7 -p1 -b .db47
%patch11 -p1 -b .create_params
%patch12 -p1 -b .delete_prefix
%patch13 -p1 -b .wcwidth
...
%if %{hcache}
		--enable-hcache \
%{!?_with_gdbm:	--without-gdbm} \
%{!?_with_qdbm:	--without-qdbm} \
%endif
%if %{imap} || %{pop} || %{smtp}
%{?gnutls:	--with-gnutls} \
%{?_with_sasl:	--with-sasl} \
%endif
%if %{imap}
%{?_with_gss: 	--with-gss} \
%endif
%{!?_with_idn:	--without-idn} \
...
%changelog
* Thu Feb 03 2011 Oota Toshiya <ribbon@users.sourceforge.jp> 5:1.5.18-5ja
- some bug fix
- apply ja patch
